...
export const calculateGoalDuration = (goal: Goal, level: UserLevel): number => {
  ...
  if (goal.type === 'AULA') {
    // Sum of subgoals
    computedDuration = goal.subGoals ? goal.subGoals.reduce((acc, sub) => acc + (sub.duration || 0), 0) : 0;
    // Fallback for Aula if empty
    if (computedDuration === 0) computedDuration = 30; 
  } else if (goal.type === 'RESUMO') {
  ...
  return isNaN(computedDuration) ? 15 : computedDuration;
};
